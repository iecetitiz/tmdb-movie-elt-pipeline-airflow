...
# Retrieve credentials from system environment variables
access_key = os.getenv("MINIO_ACCESS_KEY")
secret_key = os.getenv("MINIO_SECRET_KEY")
endpoint = os.getenv("MINIO_ENDPOINT")

# Initialize Spark Session with Delta Lake and MinIO (S3A) support
...
# Select and flatten crew fields
...
# Write processed crew data to silver layer as Delta table
crew_df.write \
    .format("delta") \
    .mode("overwrite") \
...
# Select and flatten cast fields
...
# Write processed cast data to silver layer
cast_df.write \
    .format("delta") \
    .mode("overwrite") \
...
# Select and rename core movie columns
...
# Write core movie data to silver layer
movies.write \
    .format("delta") \
    .mode("overwrite") \
...
# Select genre fields and handle missing IDs
...
# Select keyword fields and handle missing IDs
...
# Parse and explode production companies
movies_parsed = df_movies.withColumn(
...
# Parse and explode production countries
movies_parsed = df_movies.withColumn(
...
# Select production country fields and fill missing ISO codes
...
# Parse and explode spoken languages
movies_parsed = df_movies.withColumn(
...
# Select spoken language fields and fill missing codes
...
# Save final table to silver layer
